...
class PostUpdate(LoginRequiredMixin,UpdateView):
    ...
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super(PostUpdate, self).get_context_data() # PostList
...
class PostCreate(LoginRequiredMixin,UserPassesTestMixin,CreateView):
    ...
    # 템플릿 : 모델명_form.html
    def get_context_data(self, *, object_list=None, **kwargs):
        context = super(PostCreate, self).get_context_data() # PostList
        context['categories'] = Category.objects.all()
        context['no_category_post_count'] = Post.objects.filter(category=None).count
        return context
...
class PostList(ListView):
    model = Post
    ordering = '-pk'
    paginate_by = 5 #페이지 나누어서 보여주는거.. 포스트 개수
...
class PostSearch(PostList): # ListView 상속, post_list, post_list.html
    paginate_by = None #모든 포스트 보여주어야 하니까, 페이지네이션 중단

    def get_queryset(self):
        q = self.kwargs['q']
        post_list = Post.objects.filter(
            Q(title__contains=q) | Q(tags__name__contains=q)
        ).distinct() #여러개 포스트가 포함되어 있으면 그중에 하나만
        return post_list
    def get_context_data(self, *, object_list=None, **kwargs):
...
def tag_page(request, slug):
    ...
    return render(request, 'blog/post_list.html', {
        'tag' : tag,
        'post_list' : post_list,
        'categories': Category.objects.all(), # 왜 또 전달하지?
        'no_category_post_count': Post.objects.filter(category=None).count
    })
